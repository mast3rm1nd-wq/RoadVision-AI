...
# ----------------------------------------------------
# 1. PATH RESOLUTION (Local & Robust Self-Healing)
# ----------------------------------------------------
# Base path points to the directory where this script resides
...
# Create necessary local project directories
for p in [DATA, OUT, FIG, MODELS]:
    p.mkdir(parents=True, exist_ok=True)

def locate_data_file(filename, expected_rel_path):
    """
    Search for files locally, then in default datasets directories, 
    and fall back to searching recursive folders in the parent directory.
    Safely ignores filesystem root recursive walk in Colab/Linux environments.
    """
    ...
    raise FileNotFoundError(
        f"Critical file '{filename}' not found.\n"
        f"Please place the file in the same directory as this script or under: {expected_rel_path}"
    )
...
# Single-object detection baseline (keep largest box per image)
...
# Plots
labels['class'].value_counts().plot(kind='bar', figsize=(10, 5), title='All Annotation Class Distribution')
plt.tight_layout()
plt.savefig(FIG / 'annotation_class_distribution.png', dpi=150)
plt.close()

rep_sub['class'].value_counts().plot(kind='bar', figsize=(10, 5), title='Modeling Subset Class Distribution')
plt.tight_layout()
plt.savefig(FIG / 'modeling_subset_class_distribution.png', dpi=150)
plt.close()

# ----------------------------------------------------
# 3. PYTORCH MODEL PIPELINE (UPGRADED ARCHITECTURE)
# ----------------------------------------------------
class DS(Dataset):
...
class UpgradedCNN(nn.Module):
    """
    An upgraded architecture featuring Batch Normalization, Dropout, 
    and larger feature bottleneck (512 features) to improve localization & classification.
    """
    def __init__(self, num_classes):
        ...
        self.f = nn.Sequential(
            ...
            # Global Average Pooling to reduce dimensions cleanly
            nn.AdaptiveAvgPool2d((2, 2)),  # -> 2x2
            nn.Flatten()  # -> 128 * 2 * 2 = 512 dimensions
        )
        ...
        self.b = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(128, 4),
            nn.Sigmoid()  # normalized coordinates inside [0, 1]
        )
...
hist = []
epochs = 5  # increased from 2 epochs to give the model a better convergence profile
...
for _, r in test_df.sample(min(6, len(test_df)), random_state=SEED).iterrows():
    ...
    dr.text((5, 5), f"true={r['class']} pred={classes[pred_idx]} {pr[pred_idx]:.2f}", fill='yellow')
...
# ----------------------------------------------------
# 4. TESLA EDA (EXPLORATORY DATA ANALYSIS)
# ----------------------------------------------------
...
for col, fname, title, top in plots:
    if col in df:
        ...
        if len(vals) > 0:
            vals.plot(kind='bar', figsize=(10, 5), title=title)
            plt.tight_layout()
            plt.savefig(FIG / f'{fname}.png', dpi=150)
            plt.close()

# ----------------------------------------------------
# 5. PACKAGING AND SUMMARY EXPORT
# ----------------------------------------------------
...
- Modeling subset used for this local run: {summary['n_modeling_records']:,}
...
# Copy the script contents into itself (simulating self-documentation)
# Save it as run_end_to_end_local.py inside BASE
script_copy_target = BASE / 'run_end_to_end_local.py'
if script_copy_target.exists():
    # Write from local memory instead of reading __file__ directly, or read parent script
    script_copy_target.write_text(Path(__file__).read_text())

# Create a local project zip in the parent directory (Downloads)
...
with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as z:
    for path in BASE.rglob('*'):
        if path.is_file():
            ...
            # Skip heavy data inputs and temp files
            if str(rel).startswith('data/Images/') or rel.name in ['labels.csv', 'Tesla - Deaths.csv', '1739779188_capstone1problemstatement.pdf']:
                continue
